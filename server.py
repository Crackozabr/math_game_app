...
games = {}
player_wins = {}  # Глобальный словарь для хранения побед по имени игрока
...
class Game:
    ...
    def add_player(self, sid, name):
        ...
        self.players[player_id] = {
            ...
            "hand": [self.number_deck.pop(0) for _ in range(5)],
            "score_pile": [],
            "score": 0,  # Добавляем score для нового игрока
            "name": name,
            "connected": True
        }
    ...
    def process_play(self, player_id, cards, operation):
        ...
        if valid_result:
            ...
            # Проверяем, достиг ли игрок 15 очков
            ...
            "players": [
                {
                    "name": p["name"],
                    "score": p.get("score", 0),
                    "wins": player_wins.get(p["name"], 0),  # Добавляем количество побед
...
def handle_play_again(data):
    ...
    if len(games[game_id].play_again_votes) == connected_players:
        # Создаём новую игру с теми же игроками
...
def handle_join_game(data):
    ...
    for pid, player in games[game_id].players.items():
        if player["connected"]:
            socketio.emit('player_joined', {'players': [p["name"] for p in games[game_id].players.values() if p["connected"]]}, room=player['sid'])
            if pid != player_id:  # Не отправляем сообщение самому присоединившемуся игроку
...
def handle_play(data):
    ...
    if log_message:  # Успешный ход
        print(f"Успешный ход игрока {data['player_id']} в игре {game_id}")
        # Отправляем сообщение в лог всем игрокам
        for pid, player in games[game_id].players.items():
            if player["connected"]:
                socketio.emit('log', {'message': log_message}, room=player['sid'])
        if is_winner:
            winner_name = games[game_id].players[data['player_id']]['name']
            # Увеличиваем счётчик побед для победителя
            player_wins[winner_name] = player_wins.get(winner_name, 0) + 1
            winner = f"Победил {winner_name}"
            # Формируем данные о победах для модального окна
            players_wins = [
                {"name": p["name"], "wins": player_wins.get(p["name"], 0)}
...
#         # Не отправляем сообщение, если SID не принадлежит этой игре
...
def handle_disconnect():
    ...
    for game_id, game in list(games.items()):
        if game.remove_player(request.sid):
            player_name = game.players[game.get_player_id_by_sid(request.sid)]['name']
            if (time.time() - game.creation_time) > 30 and not any(p["connected"] for p in game.players.values()):
                del games[game_id]
                print(f"Игра {game_id} удалена, так как все игроки отключились")
            elif not game.game_started:
